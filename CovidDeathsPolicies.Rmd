...
- From the graph, we can say that roughly the counties with low Covid-19 death cases tend to have higher number of Covid-19 policies and counties with high Covid-19 death case tend to have lower number of Covid-19 policies. 
- However, I cannot make that assumption as a conclusion, because there are various other factors I have to consider statistically e.g. the proportion of each county (based on the number of Covid-19 policy implemented) in its respective clusters that I will do later in my analysis part.
- Also, I purposely do the color = State because I want to see if the number of Covid-19 policies depends on or have any influence correspond to the state it resides respectively. 
...
## Finding from Technique 1: Majority of the County in the United States is Implementing only 1 Covid-19 Policy
...
## Strategy 1: Use Simple Random Sampling (SRS) Method
...
- But in this project, I choose to use 'regex' with random conditions to choose the sample size, n rather than using the device or any device system to do it for me for the learning purpose experience. 
- Also, sample sizes must be equal to or greater than 30 to assume that the distribution of the sample means is fairly normally distributed.
- Thus, I will set some conditions in 'regex' with random conditions until the sample size reach 30 or more, which the sample size selected will be used for the analysis process.

*Step 1: I need to remove the "County" and "Borough" word first in each column (since I want the county/borough name solely for this entire sample size process)*
...
Condition : County which contains -three vowels or more in a row- or contains only -1 consonent- and -character 'ox' or 'pe' or 'ds' or 'zo' or 'rd'-
